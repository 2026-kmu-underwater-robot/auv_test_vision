...
echo "  ln -sfn $(cd "${SCRIPT_DIR}/.." && pwd) ~/auv_ws/src/auv_test_vision"
echo "  cd ~/auv_ws && colcon build --packages-select auv_test_vision --symlink-install"
echo "  source ~/auv_ws/install/setup.bash"
echo "  ros2 launch auv_test_vision laptop_yolo_detection.launch.py model_path:=/path/to/model.pt"
